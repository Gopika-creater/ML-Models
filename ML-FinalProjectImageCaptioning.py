...
test_model_on_image(test_data_generator, model, index_to_plot=1)
...
def generate_text(image_path, task, processor, model):
    ...
    # Instantiate the custom layer with processor and model
    blip_layer = BlipCaptionSummaryLayer(processor=processor, model=model)

    # Generate the text using the layer
    output_tensor = blip_layer(image_tensor, task_tensor)

    # Convert the result from tensor to Python string
...
caption = generate_text(image_url, "caption", processor, model)
print("Generated Caption:", caption)
image_path = tf.constant("aircraft_damage_dataset_v1/test/dent/149_22_JPG_jpg.rf.4899cbb6f4aad9588fa3811bb886c34d.jpg")  # actual path of image
# Generate a summary using the same function
summary = generate_text(image_path, "summary", processor, model)
